-- Add audit log enabled column to company table
ALTER TABLE "company"
  ADD COLUMN IF NOT EXISTS "auditLogEnabled" BOOLEAN NOT NULL DEFAULT false;

-- Create audit log archive tracking table (shared - tracks all company archives)
CREATE TABLE "auditLogArchive" (
  "id" TEXT NOT NULL DEFAULT id('arc'),
  ...
  CONSTRAINT "auditLogArchive_companyId_fkey" 
...
-- Update event system subscription handler type constraint to include AUDIT
...
CHECK ("handlerType" IN ('WEBHOOK', 'WORKFLOW', 'SYNC', 'SEARCH', 'AUDIT'));
...
DECLARE
  table_name TEXT;
BEGIN
  table_name := 'auditLog_' || p_company_id;
  ...
  EXECUTE format('
    ...
    table_name,
    table_name || '_pkey',
    table_name || '_actorId_fkey',
    table_name || '_operation_check'
  );

  -- Create indexes
  EXECUTE format('CREATE INDEX %I ON %I("entityType", "entityId")', 
    table_name || '_entityType_entityId_idx', table_name);
  EXECUTE format('CREATE INDEX %I ON %I("createdAt" DESC)', 
    table_name || '_createdAt_idx', table_name);
  EXECUTE format('CREATE INDEX %I ON %I("actorId")', 
    table_name || '_actorId_idx', table_name);
  EXECUTE format('CREATE INDEX %I ON %I("operation")', 
    table_name || '_operation_idx', table_name);
...
BEGIN
  ...
  -- Build query
  query := format('SELECT COUNT(*)::INTEGER FROM %I', table_name);
